...
-spec get(authdata_id(), storage_opts()) -> {ok, tk_storage:storable_authdata()} | {error, not_found}.
get(_DataID, _Opts) ->
    %% Collapse history and return the auth data?
    {error, not_found}.

-spec get_by_claims(claims(), storage_opts()) -> {ok, tk_storage:storable_authdata()} | {error, not_found}.
get_by_claims(_Claims, _Opts) ->
    %% Extract id from claims, collapse history and return the auth data?
    {error, not_found}.

-spec store(tk_storage:storable_authdata()) -> {ok, claims()} | {error, _Reason}.
store(_AuthData) ->
